...
*  2/28/2025 - P. Davis Edited printNode to match new requirements
*/
...
void DllNode::printNode() {
    bool verbose = false;
    if (verbose) {
        logToFileAndConsole("FIFO Node key: " + std::to_string(key) + "; name: " + fullName + ";address: " + address + "; city: " + city + "; state: " + state + "; zip: " + zip);
    }
    else {
        logToFileAndConsole("FIFO  Node key: " + std::to_string(key));
    }
}
